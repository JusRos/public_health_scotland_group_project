create_length_of_stay_plot <- function(data, health_board, 
  ...
  length_data %>% 
    ...
    geom_point(size = 3) +
    annotate("text", x = seq_len(nrow(length_data)), y = min(length_data$avg_stay) - 0.7, label = length_data$quarter) +
    annotate("text", x = 1 + 4 * (0:(length(years)-1)),
             y = min(length_data$avg_stay) - 1.3, label = years) +
    coord_cartesian(ylim = c(min(length_data$avg_stay), max(length_data$avg_stay)), clip = "off") +
