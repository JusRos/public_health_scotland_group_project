...
ggplot(aes(x = interaction(Quarter_single, Year), y = Sex, group = Sex, colour = Sex)) +
...
```{r}
age_and_sex %>% filter(Sex == "Male" & admissionType == "Emergency" group_by(Quarter, Sex) %>% summarise(number_admissions = sum(Stays)) %>% 
  ggplot()+
  geom_line(aes(x = Quarter, y = number_admissions, group = Sex, colour = Sex), size = 1.5)+
theme(axis.text.x = element_text(angle = 45, hjust = 1))+
   labs(
...
simd %>% distinct(SIMD)
```
emergency stays across different age groups vs planned stays across different age groups.