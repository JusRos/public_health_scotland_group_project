...
```{r}
# pre_covid <- age_and_sex %>% mutate(pre_covid = if_else(Year == c(2017, 2018, 2019), TRUE, FALSE))
# 
# before_covid <- pre_covid %>% filter(pre_covid == TRUE) %>% group_by(Quarter, Age) %>% summarise(number_admissions = sum(Stays))
# after_covid <- pre_covid %>% filter(pre_covid == FALSE) %>% group_by(Quarter, Age) %>% summarise(number_admissions = sum(Stays))
# before_covid %>% filter(Age=="0-9 years") %>% 
#   ggplot()+
#   geom_col(aes(x = Age, y = number_admissions))+
# theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
#pre-covid and post-covid - doesn't make sense
